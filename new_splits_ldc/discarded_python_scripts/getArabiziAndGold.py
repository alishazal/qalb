...
def handleTokenSeparation(ppw, gold):
    gold = gold.replace("[-]#", " [+]#")
    ...
    for source in root.findall("./su/source"):
        ...
        for j in range(tracker, tracker + numLines):
            if "tag" in allWords[j].attrib and allWords[j].attrib["tag"] in ["punctuation"]:
                ...
                currGoldLine[wordsOfThisLineCounter] = changeForeignsToHash(preprocessor(allWords[j].text))
            else:
                currGoldLine[wordsOfThisLineCounter] = handleTokenSeparation(preprocessor(allWords[j].text), currGoldLine[wordsOfThisLineCounter])
