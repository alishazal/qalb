"""
Usage: python3 getSourceAndTarget.py [folder] [folder/source.arabizi] [folder/ml-target.gold] [folder/final-target.gold]
"""
...
for i in dirs:
    ...
    for source in root.findall("./su/source"):
        ...
        if len(source.text.strip().split()) != len(currGoldLine):
            pass
            # print(source.text.strip().split(), currGoldLine, len(source.text.strip().split()), len(currGoldLine), i)
