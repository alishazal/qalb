"""
Usage: python3 mle-context-1-backoff.py [train.arabizi] [train.gold] [dev.arabizi] [pred.out] [unknown.out]
"""
import sys
import time
...
for line in range(len(trainA)):
    currArabiziLine = ["<bos>"] + trainA[line].strip().split() + ["<eos>"]
    currGoldLine = trainG[line].strip().split()

    count = 0
    # go over every word of line
    for word in range(1, len(currArabiziLine) - 1):
...
for line in range(len(devA)):
    currArabiziLine = ["<bos>"] + devA[line].strip().split() + ["<eos>"]
    ans = []
    unk = []
    ...
    for word in range(1, len(currArabiziLine) - 1):
        ...
        if currWordPlusMinusOne in mlePlusMinusOne:
