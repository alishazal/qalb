"""
Usage: python3 mle-context-2-backoff.py [train.arabizi] [train.gold] [dev.arabizi] [pred.out] [unknown.out]
"""
import sys
import time
...
for line in range(len(trainA)):
    currArabiziLine = ["<bos>", "<bos>"] + trainA[line].strip().split() + ["<eos>", "<eos>"]
    currGoldLine = trainG[line].strip().split()

    count = 0
    # go over every word of line
    for word in range(2, len(currArabiziLine) - 2):
...
for line in range(len(devA)):
    currArabiziLine = ["<bos>", "<bos>"] + devA[line].strip().split() + ["<eos>", "<eos>"]
    ans = []
    unk = []
    ...
    for word in range(2, len(currArabiziLine) - 2):
        ...
        if currWordPlusMinusTwo in mlePlusMinusTwo:
